...
export function CategoryDetail({ category, initialPage }: { category: CategorySummary; initialPage: WorkspacePage }) {
  ...
  async function create(event: React.FormEvent) {
    ...
    try { const workspace = await createProject(title.trim(), category.id); setTitle(""); await router.invalidate(); await navigate({ to: "/projects/$projectId", params: { projectId: workspace.id } }); }
  ...
  async function removeWorkspace(workspace: ProjectSummary) { if (!window.confirm(`Delete “${workspace.title}”?`)) return; try { await deleteProject(workspace.id); await router.invalidate(); } catch (err) { setError(err instanceof Error ? err.message : "Could not delete workspace."); } }
