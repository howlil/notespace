...
export function Dashboard({ categories, workspaces }: { categories: CategorySummary[]; workspaces: ProjectSummary[] }) {
  ...
  return (
    <div className="dashboard-shell">
      ...
      <Dialog.Root open={!!createTarget} onOpenChange={(open) => { if (!open && !busy) setCreateTarget(null); }}><Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="dialog-content create-dialog"><Dialog.Title>{createTarget?.kind === "category" ? "New category" : "New workspace"}</Dialog.Title><Dialog.Description>Choose a name.</Dialog.Description><form onSubmit={create}><input className="seamless-input" aria-label={createTarget?.kind === "category" ? "Category title" : "Workspace title"} autoFocus autoComplete="off" placeholder={createTarget?.kind === "category" ? "Category name" : "Workspace name"} maxLength={160} required value={title} onChange={(event) => setTitle(event.target.value)} /><p className="form-error" role="alert">{error}</p><div className="dialog-actions"><Dialog.Close className="secondary" disabled={busy}>Cancel</Dialog.Close><button className="primary" disabled={busy || !title.trim()}>{busy ? "Creating…" : createTarget?.kind === "category" ? "Create category" : "Create workspace"}</button></div></form></Dialog.Content></Dialog.Portal></Dialog.Root>
