...
export function CategoryLibrary({ category, initialPage }: { category: CategorySummary; initialPage: WorkspacePage }) {
  ...
  async function create(event: React.FormEvent) {
    ...
    setCreating(true);
    try { const workspace = await createWorkspace(title.trim(), category.id); setTitle(""); await router.invalidate(); await navigate({ to: "/workspaces/$workspaceId", params: { workspaceId: workspace.id } }); }
  ...
  function beginWorkspaceRename(workspace: WorkspaceSummary) { setEditingWorkspace(workspace.id); setWorkspaceTitle(workspace.title); }
  async function saveWorkspace() { const nextTitle = workspaceRenameTitle(workspaceTitle); if (!editingWorkspace || !nextTitle) return; try { await renameWorkspace(editingWorkspace, nextTitle); setEditingWorkspace(null); await router.invalidate(); } catch (err) { showToast({ kind: "error", message: errorMessage(err, "Could not rename workspace.") }); } }
  async function removeWorkspace() { if (!deletingWorkspace) return; const workspace = deletingWorkspace; setDeletingWorkspace(null); try { await deleteWorkspace(workspace.id, workspace.version); await router.invalidate(); } catch (err) { showToast({ kind: "error", message: errorMessage(err, "Could not delete workspace.") }); } }
