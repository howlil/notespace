...
const INBOX_PLANNING = join(WEB_SRC, "features", "planning", "InboxPlanning.tsx");
const TODAY = join(WEB_SRC, "pages", "today", "TodayPage.tsx");
const TODAY_PLANNING = join(WEB_SRC, "features", "planning", "TodayPlanning.tsx");
const ACTIVITY_QUICK_START = join(WEB_SRC, "features", "activity", "ActivityQuickStart.tsx");
const PANE_LAYOUT = join(WEB_SRC, "features", "workspace-authoring", "model", "pane-layout.ts");
const WORKSPACE_CONTENT = join(WEB_SRC, "features", "workspace-authoring", "model", "workspace-content.ts");
...
const CANVAS_SCENE_STATE = join(WEB_SRC, "features", "workspace-authoring", "canvas", "canvas-scene-state.ts");
const IMAGE_ASSET_URL = join(WEB_SRC, "features", "workspace-authoring", "assets", "use-image-asset-url.ts");
const CANVAS_CHROME = join(WEB_SRC, "features", "workspace-authoring", "canvas", "CanvasChrome.tsx");
const CANVAS_NATIVE_ACTIONS = join(WEB_SRC, "features", "workspace-authoring", "canvas", "CanvasNativeActions.ts");
...
const CANVAS_PEER_CHANNEL = join(WEB_SRC, "features", "workspace-authoring", "canvas", "use-canvas-peer-channel.ts");
const CANVAS_CODE_BLOCK = join(WEB_SRC, "features", "workspace-authoring", "canvas", "canvas-code-block.ts");
const CANVAS_CODE_BLOCK_LAYER = join(WEB_SRC, "features", "workspace-authoring", "canvas", "CanvasCodeBlockLayer.tsx");
const CANVAS_CODE_BLOCK_ACTIONS = join(WEB_SRC, "features", "workspace-authoring", "canvas", "CanvasCodeBlockActions.tsx");
const CANVAS_CODE_BLOCK_MEASURE = join(WEB_SRC, "features", "workspace-authoring", "canvas", "CanvasCodeBlockMeasure.tsx");
const CANVAS_CODE_BLOCK_LAYOUT = join(WEB_SRC, "features", "workspace-authoring", "canvas", "canvas-code-block-layout.ts");
const CANVAS_CODE_RUNNER = join(WEB_SRC, "features", "workspace-authoring", "canvas", "canvas-code-runner.ts");
const CANVAS_CODE_RUNNER_HOOK = join(WEB_SRC, "features", "workspace-authoring", "canvas", "use-canvas-code-runner.ts");
const CODE_LANGUAGE = join(WEB_SRC, "domain", "code", "code-language.ts");
const CODE_RUNNER = join(WEB_SRC, "features", "workspace-authoring", "code", "code-runner.ts");
const CODE_RUNNER_HOOK = join(WEB_SRC, "features", "workspace-authoring", "code", "use-code-runner.ts");
...
test("frontend contract: repeated page controls reuse shared UI primitives", () => {
  ...
  assert.match(source(WORKSPACE_LIBRARY), /<Button/); assert.match(source(SIDEBAR), /<Button/);
...
test("frontend styling contract: application surfaces are utility-first", () => {
  for (const file of [ROUTE_PENDING, DASHBOARD, WORKSPACE_LIBRARY, SIDEBAR, QUICK_CAPTURE, LIBRARY_TOOLS, WORKSPACE, WORKSPACE_PLAN, INBOX, INBOX_PLANNING, TODAY, TODAY_PLANNING, ACTIVITY_QUICK_START, DOCUMENT_EDITOR, CANVAS, CANVAS_CHROME, CANVAS_SELECTION_ACTIONS, TOAST_PROVIDER, ACTIVITY_DASHBOARD, ACTIVITY_INDICATOR, ACTIVITY_TYPE_MENU, ACTIVITY_TYPE_TRIGGER]) {
...
test("workspace contract: bounded panes and explicit Canvas frame embeds remain user-facing behavior", () => {
  ...
  assert.match(source(INBOX_PLANNING), /Capture first\. Decide when to do it later\./);
  assert.match(source(INBOX_PLANNING), /New inbox task/);
  assert.match(source(INBOX_PLANNING), /Move .* to Today/);
  assert.doesNotMatch(source(INBOX_PLANNING), /priority|kanban|milestone|recurrence|productivity score/i);
  assert.match(source(WORKSPACE_PLAN), /Add .* to Today/);
  assert.match(source(TODAY_PLANNING), /One place for the work you explicitly chose to do today/);
  assert.match(source(TODAY_PLANNING), /Standalone/);
  assert.match(source(TODAY_PLANNING), /Move .* to Inbox/);
  assert.match(source(ACTIVITY_QUICK_START), /What are you doing\?/);
  assert.match(source(ACTIVITY_QUICK_START), /Activity type/);
  ...
  assert.match(source(TODAY_PLANNING), /getToday/);
  ...
  assert.doesNotMatch(source(TODAY_PLANNING), /priority|kanban|habit|streak|productivity score/i);
  assert.match(source(ACTIVITY_INDICATOR), /aria-label="Start activity"/);
  assert.match(source(ACTIVITY_INDICATOR), /ActivityTypeMenu/);
  assert.doesNotMatch(source(ACTIVITY_INDICATOR), /taskHandoff|onEnd|activity\.pause|activity\.resume|activity\.end/);
  assert.match(source(WORKSPACE_PAGE), /useActivityRuntime/);
  assert.match(source(WORKSPACE_PAGE), /adoptLegacyWorkspace/);
  assert.match(source(WORKSPACE_PAGE), /refreshKey=\{activity\.taskRevision\}/);
...
test("asset contract: server is durable owner and IndexedDB is only a compatibility cache", () => {
  ...
  assert.match(api,/\/api\/workspaces\/\$\{encodeURIComponent\(workspaceId\)\}\/assets/); assert.match(api,/method:\s*"PUT"/); assert.match(api,/loadRemoteImageAsset/); assert.match(store,/createImageStore/); assert.match(core,/Read-through migration/); assert.match(cache,/indexedDB\.open\(DATABASE_NAME/); assert.match(core,/inFlightLoads/);
});
