...
export function StudyIndicator({ study }: { study: StudySessionState }) {
  ...
  return <div className="study-indicator-wrap">
    ...
    {open && <div className="study-popover" role="dialog" aria-label="Study activity">
      ...
      <div className="study-popover-footer"><span><i className={study.tracking ? "study-live-dot" : "study-paused-dot"}>●</i> {state}</span><button className="secondary compact-action" onClick={study.tracking ? study.pause : study.resume}>{study.tracking ? <><Pause size={13} /> Pause</> : <><Play size={13} /> Resume</>}</button></div>
    </div>}
  </div>;
}
