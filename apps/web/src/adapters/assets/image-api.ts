...
export function createImageApi(fetcher: typeof fetch = globalThis.fetch) {
  return {
    async uploadRemoteImageAsset(
      workspaceId: string,
      id: string,
      blob: Blob,
    ) {
      const response = await fetcher(assetUrl(workspaceId, id), {
        method: "PUT",
...
const defaultImageApi = createImageApi();

export const uploadRemoteImageAsset = defaultImageApi.uploadRemoteImageAsset;
export const loadRemoteImageAsset = defaultImageApi.loadRemoteImageAsset;
