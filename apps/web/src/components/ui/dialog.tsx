...
export function DialogContent({ className, children, ...props }: ComponentPropsWithoutRef<typeof DialogPrimitive.Content>) {
  return (
    <DialogPrimitive.Portal>
      <DialogPrimitive.Overlay className="fixed inset-0 z-[110] bg-black/35 backdrop-blur-[2px]" />
      <DialogPrimitive.Content asChild {...props}>
        <motion.div
          ...
          className={cn(
            "fixed left-1/2 top-1/2 z-[120] max-h-[calc(100dvh_-_24px)] w-[min(92vw,420px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto overscroll-contain rounded-xl border border-[var(--line)] bg-[var(--surface)] p-5 text-[var(--ink)] shadow-2xl shadow-black/20 focus:outline-none max-[480px]:w-[calc(100vw_-_24px)] max-[480px]:p-4",
            className,
          )}
        >
          {children}
