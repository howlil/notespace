...
export function Dashboard({ categories, recentWorkspaces, initialSelectedCategoryId, initialCategoryPage }: Props) {
  ...
  async function selectCategory(id: string, force = false) {
    ...
    setSelectedCategoryId(id); setView("category"); setPageLoading(true); setPageError("");
    try { const result = await listCategoryWorkspaces(id, { limit: 50 }); setPage(result); }
    catch (err) { setPageError(err instanceof Error ? err.message : "Could not load category workspaces."); }
    finally { setPageLoading(false); }
  }

  async function openAll() {
    setView("all"); setSelectedCategoryId(""); setPageLoading(true); setPageError("");
    try { const result = await listAllWorkspaces({ limit: 50 }); setPage(result); }
    catch (err) { setPageError(err instanceof Error ? err.message : "Could not load workspaces."); }
    finally { setPageLoading(false); }
  }
  ...
  return <div className={collapsed ? "app-shell dashboard-app-shell sidebar-collapsed" : "app-shell dashboard-app-shell"}><Sidebar categories={categories} selectedCategoryId={selectedCategoryId} collapsed={collapsed} onToggle={() => setCollapsed((value) => !value)} onSelectCategory={(id) => void selectCategory(id)} onChanged={refreshLibrary} /><main className="dashboard">
    ...
    <div className="dashboard-content">
      ...
      <section className="library-main-pane" aria-labelledby="library-list-title">{pageError && <p className="dashboard-inline-error" role="alert">{pageError}</p>}{pageLoading ? <p className="library-loading">Loading workspaces…</p> : items.length ? <div className="workspace-library-list">{items.map((workspace) => <Link key={workspace.id} to="/workspaces/$workspaceId" params={{ workspaceId: workspace.id }} className="workspace-library-row"><span className="workspace-row-icon"><FileText size={16} /></span><span className="workspace-row-copy"><strong>{workspace.title}</strong><span>{view === "all" ? `${categories.find((category) => category.id === workspace.categoryId)?.title ?? "Category"} · ` : ""}{workspace.noteCount ?? 0} note{workspace.noteCount === 1 ? "" : "s"}{workspace.hasCanvas ? " · Canvas" : ""}</span></span><time dateTime={workspace.updatedAt}>{editedAt(workspace.updatedAt)}</time></Link>)}</div> : <div className="empty-state category-empty"><span className="empty-mark"><Folder size={22} /></span><h2>{view === "recent" ? "No recent workspaces" : "No workspaces here"}</h2><p>Create a workspace from the sidebar.</p></div>}</section>
