...
const TODAY = join(WEB_SRC, "features", "today", "Today.tsx");
...
const ANCHORED_PANEL = join(WEB_SRC, "components", "ui", "anchored-panel.ts");
...
test("frontend styling contract: application surfaces are utility-first", () => {
  for (const file of [ROUTE_PENDING, DASHBOARD, SIDEBAR, QUICK_CAPTURE, LIBRARY_TOOLS, WORKSPACE, WORKSPACE_PLAN, TODAY, DOCUMENT_EDITOR, CANVAS, CANVAS_CHROME, CANVAS_SELECTION_ACTIONS, TOAST_PROVIDER, STUDY_ACTIVITY, STUDY_INDICATOR]) {
...
test("workspace contract: bounded panes and explicit Canvas frame embeds remain user-facing behavior", () => {
  ...
  assert.match(source(WORKSPACE_PLAN), /ActivityTypeMenu/);
  ...
  assert.match(source(STUDY_INDICATOR), /Choose activity type/);
  assert.match(source(STUDY_INDICATOR), /Build/);
  assert.match(source(STUDY_INDICATOR), /Learn/);
  assert.match(source(STUDY_INDICATOR), /Read/);
  assert.match(source(STUDY_INDICATOR), /Write/);
  assert.match(source(STUDY_INDICATOR), /Exercise/);
  assert.match(source(STUDY_INDICATOR), /Other/);
