...
func granularDocument(text string) workspacepkg.Snapshot {
	...
	return workspacepkg.Snapshot{Format: "tiptap", Version: 1, Data: data}
}

func granularCanvas(id string) workspacepkg.Snapshot {
	data, _ := json.Marshal(map[string]any{
		"elements": []any{map[string]any{"id": id, "type": "rectangle"}},
		"appState": map[string]any{},
		"files":    map[string]any{},
	})
	return workspacepkg.Snapshot{Format: "excalidraw", Version: 1, Data: data}
}
...
func TestGranularWorkspaceStateHasIndependentVersions(t *testing.T) {
	...
	created, err := service.CreateNote(ctx, workspace.ID, workspacepkg.NoteCreate{
	...
	updated, err := service.UpdateNote(ctx, workspace.ID, created.ID, workspacepkg.NoteUpdate{
	...
	if _, err := service.UpdateNote(ctx, workspace.ID, created.ID, workspacepkg.NoteUpdate{
		Title: "stale", Document: granularDocument("stale"), Version: created.Version,
	}); !errors.Is(err, workspacepkg.ErrConflict) {
		t.Fatalf("stale note update error = %v, want conflict", err)
	}

	canvas, err := service.UpdateCanvas(ctx, workspace.ID, workspacepkg.CanvasUpdate{
	...
	if _, err := service.UpdateCanvas(ctx, workspace.ID, workspacepkg.CanvasUpdate{
		Canvas: granularCanvas("stale"), Version: workspace.CanvasVersion,
	}); !errors.Is(err, workspacepkg.ErrConflict) {
		t.Fatalf("stale canvas update error = %v, want conflict", err)
	}

	page, err := service.ListWorkspaces(ctx, workspacepkg.WorkspaceQuery{Offset: 0, Limit: 50, HasCanvas: true, HasNotes: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := service.DeleteNote(ctx, workspace.ID, created.ID, created.Version); !errors.Is(err, workspacepkg.ErrConflict) {
...
func TestGranularWriteInvalidatesStaleAggregateSnapshot(t *testing.T) {
	...
	savedNote, err := service.UpdateNote(ctx, workspace.ID, note.ID, workspacepkg.NoteUpdate{
	...
	_, err = service.Update(ctx, workspace.ID, workspacepkg.Update{
