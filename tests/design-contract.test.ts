...
test("canvas contract: tool, contextual, and viewport chrome have distinct ownership", () => {
  ...
  const codeBlock = source(CANVAS_CODE_BLOCK), codeLayer = source(CANVAS_CODE_BLOCK_LAYER), codeActions = source(CANVAS_CODE_BLOCK_ACTIONS), codeMeasure = source(CANVAS_CODE_BLOCK_MEASURE), codeLayout = source(CANVAS_CODE_BLOCK_LAYOUT), codeRunnerHook = source(CANVAS_CODE_RUNNER_HOOK);
  ...
  assert.match(codeActions, /Edit code/);
